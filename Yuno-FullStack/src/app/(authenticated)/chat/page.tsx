...
export default function ChatPage() {
  ...
  useEffect(() => {
    const loadChatUsers = async () => {
      try {
        const response = await fetch(`/api/discover?radius=20`, {
          headers: {
        ...
        if (response.ok) {
          ...
          const usersData = data.data && Array.isArray(data.data) ? data.data : [];
          ...
          console.error('Failed to load chat users:', response.status);
          // Fallback to mock data
          setChatUsers(getMockChatUsers());
        }
      } catch (error) {
  ...
  const getMockChatUsers = (): ChatUser[] => [
    {
      id: '1',
      name: 'Priya Sharma',
  ...
  useEffect(() => {
    ...
    // Dynamic Socket.io URL that works in both development and production
    const getSocketUrl = () => {
      ...
      return process.env.NEXT_PUBLIC_SOCKET_URL || 'https://yuno-full-stack-eu1yr41uh-aakarsh12xs-projects.vercel.app';
    };
  ...
  const selectChat = async (chatId: string) => {
    ...
    try {
              const response = await fetch(`/api/chat/chats/${chatId}?limit=50`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('accessToken')}`,
          'Content-Type': 'application/json'
        }
